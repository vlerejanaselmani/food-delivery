...
export default function AuthModal({ cities, onClose, onSuccess }) {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      const { user } = await api(register ? "register" : "login", {
        method: "POST",
  ...
  return (
    <div className="modal-backdrop">
      ...
      >
        ...
        <h2 id="auth-title">
          {register ? "Join the table." : "Welcome back."}
        </h2>
        <p className="muted">
          {register
            ? "Save your favorites. Keep every delicious memory."
        ...
        <form onSubmit={submit}>
          ...
          {register && (
            <div className="form-row">
              <label>
                City
                <select name="city" defaultValue="Prishtinë">
                  {cities.map((c) => (
